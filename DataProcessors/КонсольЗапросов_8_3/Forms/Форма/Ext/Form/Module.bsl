...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	Если ЭлементДерева <> Неопределено Тогда
		...
		ПараметрыКонструктора = Новый Структура("Конструктор, ЭлементДерева", Конструктор, ЭлементДерева);
		ОповещениеКонструктора = Новый ОписаниеОповещения("ВыполнитьПослеЗакрытияКонструктора", ЭтотОбъект, ПараметрыКонструктора);
...
Процедура ВыполнитьПослеЗакрытияКонструктора(Результат, ПараметрыКонструктора) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстЗапроса) Тогда  
		ПараметрыКонструктора.ЭлементДерева.ТекстЗапроса = ТекстЗапроса;
...
Процедура ЗаполнитьПараметры(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТекущиеДанные.ПараметрыЗапроса.Количество() > 0 Тогда
			Если НЕ Вопрос("Список параметров будет очищен. Продолжить?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
