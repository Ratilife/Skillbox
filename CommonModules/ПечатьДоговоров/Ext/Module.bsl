...
Функция ФайлДоговораДляОбъекта(СсылкаНаОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
		Результат = ОбщееНазначение.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента, "ФайлДоговора");
	КонецЕсли;
...
Функция ДоговорКонтрагентаДляОбъекта(СсылкаНаОбъект) Экспорт
	...
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.ДоговорыКонтрагентовПрисоединенныеФайлы") Тогда
		Результат = ОбщееНазначение.ЗначениеРеквизитаОбъекта(СсылкаНаОбъект, "ВладелецФайла");
	Иначе
		...
		Если МетаданныеОбъекта <> Неопределено Тогда
			Если МетаданныеОбъекта.Реквизиты.Найти("ДоговорКонтрагента") <> Неопределено Тогда
				Результат = ОбщееНазначение.ЗначениеРеквизитаОбъекта(СсылкаНаОбъект, "ДоговорКонтрагента");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция ЗначениеПараметра(Знач ИмяПараметра, СведенияОСделке, ЗначенияСоставногоПараметра, Успешно) Экспорт
	...
	ИначеЕсли ИмяПараметра = Врег("СловоИменуемоеДляКонтрагента") Тогда
		...
		Если ЗначениеЗаполнено(СведенияОКонтрагенте.ИндивидуальныйПредприниматель) Тогда
			ПолФизическогоЛица = ОбщееНазначение.ЗначениеРеквизитаОбъекта(
...
Процедура ДопзаполнитьНаименованиеДопСвойств(ТаблицаДопСвойств)
	...
	ОписаниеТипаСтрока = ОбщееНазначение.ОписаниеТипаСтрока(Метаданные.ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.ДлинаНаименования);
	ТаблицаДопСвойств.Колонки.Добавить("НаименованиеСвойства", ОписаниеТипаСтрока);
	
	МассивСвойств = ОбщееНазначение.ВыгрузитьКолонку(ТаблицаДопСвойств, "Свойство");
	РеквизитыСвойств = ОбщееНазначение.ЗначенияРеквизитовОбъектов(МассивСвойств, "Наименование");
...
Функция ПредставлениеДопСвойства(СтрокаТаблицыДопСвойств)
	...
	Иначе
		...
		СведенияОДопСвойстве = ОбщееНазначение.ЗначенияРеквизитовОбъекта(СтрокаТаблицыДопСвойств.Свойство, "ФорматСвойства");
